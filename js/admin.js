...
let thirdPartyCache = []; // Cache para armazenar avaliações e permitir filtro local
...
// ============================================================
// 10. GESTÃO DE TERCEIRIZADA (NOVO MÓDULO)
// ============================================================
...
if (formThirdParty) {
    formThirdParty.addEventListener('submit', async (e) => {
        e.preventDefault();
        ...
        try {
            ...
            await setDoc(newDocRef, {
                ...
                date,
                agent,
                company, // Salva no Firebase
                comment,
                status: statusEl.value,
                createdAt: new Date(),
                auditor: document.getElementById('admin-name').innerText 
            });

            alert("Avaliação registrada com sucesso!");
            formThirdParty.reset();
        } catch (error) {
            console.error(error);
...
// B. Carregar Dashboard da Terceirizada
window.loadThirdPartyDashboard = async () => {
    ...
    try {
        ...
        // 2. Calcula KPIs e Gráfico de Status
        updateThirdPartyKPIsLogic(); 

        // 3. Calcula e Renderiza Gráfico Temporal (NOVO)
...
// Nova Função: Processamento e Renderização do Gráfico de Linhas
function processAndRenderTimeline() {
    const ctx = document.getElementById('chartThirdPartyTimeline');
    if (!ctx) return;

    // A. Agrupamento por Data
    const groups = {};
    
    thirdPartyCache.forEach(item => {
        const d = item.date; // Formato YYYY-MM-DD
        if (!groups[d]) {
            groups[d] = { conf: 0, leve: 0, grave: 0 };
        }
    ...
    // B. Ordenação Cronológica (Crescente)
    const sortedDates = Object.keys(groups).sort();

    // C. Preparação dos Arrays para o Chart.js
    const labels = sortedDates.map(date => date.split('-').reverse().slice(0,2).join('/')); // dd/mm
    const dataConf = sortedDates.map(d => groups[d].conf);
    const dataLeve = sortedDates.map(d => groups[d].leve);
    const dataGrave = sortedDates.map(d => groups[d].grave);
...
// Nova Função: Renderiza a tabela baseada no Cache e no Input de Pesquisa
window.renderThirdPartyTable = () => {
    const tableBody = document.getElementById('table-3p-body');
    const searchTerm = document.getElementById('search-3p-protocol').value.trim().toLowerCase();
...
// Nova Função: Lógica isolada de KPIs (para organizar o código)
...
// Nova Função: Abre o Modal de Detalhes
// Nova Função: Abre o Modal de Detalhes (Atualizado com Empresa)
window.viewThirdPartyDetail = (id) => {
    ...
    // Layout do Modal Atualizado com Empresa
    modalContent.innerHTML = `
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 20px;">
            <div>
                <label style="font-size:11px; color:#666; font-weight:bold;">PROTOCOLO</label>
                <div style="font-size:16px; color:#333;">${item.protocol}</div>
            </div>
            <div>
                <label style="font-size:11px; color:#666; font-weight:bold;">EMPRESA</label>
                <div style="font-size:16px; color:#333; font-weight:600;">${item.company || 'N/A'}</div>
            </div>
            <div>
                <label style="font-size:11px; color:#666; font-weight:bold;">DATA</label>
                <div style="font-size:16px; color:#333;">${item.date.split('-').reverse().join('/')}</div>
            </div>
            <div>
                <label style="font-size:11px; color:#666; font-weight:bold;">ATENDENTE</label>
                <div style="font-size:16px; color:#333;">${item.agent}</div>
            </div>
        </div>
        
        <div style="margin-bottom: 15px;">
             <label style="font-size:11px; color:#666; font-weight:bold;">AUDITOR</label>
             <div style="font-size:14px; color:#555;">${item.auditor || 'Não informado'}</div>
        </div>

        <div style="background: ${statusColor}20; padding: 15px; border-left: 5px solid ${statusColor}; border-radius: 4px; margin-bottom: 20px;">
            <h4 style="margin: 0; color: ${statusColor};">${statusText}</h4>
        </div>

        <div>
            <label style="font-size:12px; color:#666; font-weight:bold; display:block; margin-bottom: 5px;">COMENTÁRIO / ANÁLISE COMPLETA</label>
            <div style="background: #f8f9fa; padding: 15px; border: 1px solid #eee; border-radius: 6px; color: #444; line-height: 1.5;">
                ${item.comment}
            </div>
        </div>
    `;
...
function updateThirdPartyKPIs(total, conf, leve, grave) {
    document.getElementById('kpi-3p-total').innerText = total;
    document.getElementById('kpi-3p-leve').innerText = leve;
    document.getElementById('kpi-3p-grave').innerText = grave;
    
    const perc = total > 0 ? ((conf / total) * 100).toFixed(1) : 0;
    document.getElementById('kpi-3p-conformidade').innerText = perc + "%";
}

let chart3PInstance = null;

function renderThirdPartyChart(conf, leve, grave) {
    const ctx = document.getElementById('chartThirdPartyStatus');
    if (!ctx) return;

    if (chart3PInstance) chart3PInstance.destroy();

    chart3PInstance = new Chart(ctx, {
        type: 'doughnut',
        data: {
